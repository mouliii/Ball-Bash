...
onready var forcePush = get_parent().get_node("ForcePush")
...
func _physics_process(delta):
	...
	if parent.position_id  <= 1:
		...
		if target:
			...
			if target.global_transform.origin.x > global_transform.origin.x + cartOffset:
				pass#input = 1
			else:
				pass#input = -1
		else:
			# asd
			targetPos = Vector3(0,global_transform.origin.y, global_transform.origin.z)
			if global_transform.origin.distance_to(Vector3(0,global_transform.origin.y,global_transform.origin.z)) > 0.2:
				if 0 > global_transform.origin.x:
					pass#input = 1
				else:
					pass#input = -1
	# sivu pelaajat
		...
		if target:
			# asd
			targetPos = target.global_transform.origin
			if target.global_transform.origin.z > global_transform.origin.z + cartOffset:
				pass#input = 1
			else:
				pass#input = -1
		else:
			# asd
			targetPos = Vector3(global_transform.origin.x,global_transform.origin.y, 0)
			if global_transform.origin.distance_to(Vector3(global_transform.origin.x,global_transform.origin.y,0)) > 0.2:
				if 0 > global_transform.origin.z:
					pass#input = 1
				else:
					pass#input = -1
	...
	if parent.position_id  <= 1:
		...
		var newpos = parent.global_transform.origin.move_toward( \
			Vector3(global_transform.origin.x, parent.global_transform.origin.y, targetPos.z), parent.speed * delta * delta)
